...
def train_shapes(shapes,qnt_shapes,tans,limit):
    ...
    if len(shapes) > 0 and min(dists) < limit:
        ...
        qnt_shapes[i] += 1
        return shapes,qnt_shapes,i
    else:
        shapes.append(tans)
        qnt_shapes.append(1)
        return shapes,qnt_shapes, len(shapes)
...
if __name__ == '__main__':
    ...
    black = np.zeros((w+2,h+2,c)).astype(int)
    ...
    for col in range(1,9):
        ...
        for path in paths:
            t = tangents(path,20)
            tans += [t]
            patterns,qnt,pat = train_shapes(patterns,qnt,t,2)
            for p in path:
                black[p] = colors[pat%14]
